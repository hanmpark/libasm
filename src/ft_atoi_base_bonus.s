...
ft_atoi_base:	; rdi = char *str, rsi = char *base
	...
	.check_base:
		cmp BYTE [rsi + rcx], 0			; if (base[rcx] == '\0')
		jz .check_base_final
		mov r8, rcx						; r8 = int j

	.check_base_duplicate:
		inc r8							; r8 == j + 1
		cmp BYTE [rsi + r8], 0			; if (base[j + 1] == '\0')
		jz .check_base_character_format
		mov r10b, BYTE [rsi + r8]
		cmp BYTE [rsi + rcx], r10b		; if (base[j] == base[r8])
		je .return
	...
	.check_sign:
		cmp BYTE [rdi + r8], 45			; if (str[r8] == '-')
		je .is_negative
		cmp BYTE [rdi + r8], 43			; if (str[r8] == '+')
		je .is_positive
		jmp .atoi
	...
	.atoi_find_in_base:
		mov r10b, BYTE [rsi + r9]		; r10b = base[r9]
		cmp r10b, 0						; if (base[r9] == '\0')
		je .return
		cmp BYTE [rdi + r8], r10b		; if (str[r8] != base[r9])
		jne .atoi_find_in_base_inc

	.atoi_add_to_total:
		mul rcx							; rax = rax * rcx (length of base)
